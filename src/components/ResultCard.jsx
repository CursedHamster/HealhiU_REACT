...
function ResultCard(props) {
  ...
  const { bmi, testResult, goodProducts, badProducts, calories } =
    props.resultInfo;
  ...
  return (
    <div className={`result-card d-flex flex-column mh-100 rounded-5 d-lg-grid ${cardType && cardType === cardTypes[1] ? 'type-d' : 'type-s'}`}>
      <div className="bmi-cell w-100 d-flex align-items-center">
        <h3>{bmiLabel}</h3>
      ...
      <div className="right d-flex flex-column">
        <h3>{caloriesLabel}</h3>
        <div className="calories w-100 d-flex flex-column gap-3">
          {calories.maintain && (
      ...
      {cardType && cardType === cardTypes[1] && (
        <div className="description-result d-flex flex-column">
          <h3>{resultLabel}</h3>
